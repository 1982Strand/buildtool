...
cd $IN
find $IN -name *.mrm -type f|xargs sed -i 's/默认/Standard/g'
...
cd $IN
find $IN/system/build.prop -type f|xargs sed -i 's/ro.product.locale.language=zh/ro.product.locale.language=da/g'
find $IN/system/build.prop -type f|xargs sed -i 's/ro.product.locale.region=CN/ro.product.locale.region=DK/g'
cd $HOME
...
mv -f $OUT/miui_i9300_${ver}.zip $OUT/miui_i9300_${ver}_DA.zip
